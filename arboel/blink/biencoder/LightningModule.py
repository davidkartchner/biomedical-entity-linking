...
def filter_by_context_doc_id(mention_idxs, doc_id, doc_id_list, return_numpy=False):
    ...
    print("\nlen(doc_id_list) : ", len(doc_id_list))
    # print("\nmention_idxs :", mention_idxs)
    print("\nmax(mention_idxs) :", max(mention_idxs))
    # print("------------------")
    ...
    def on_validation_epoch_start(self):
        ...
        if not self.hparams["use_types"]:  # Only one index so only need one search
            ...
            for entity_type in self.valid_men_indexes:
                ...
                self.valid_nn_ent_idxs_by_type = np.array(  # CC4 DD8
                    list(
                        map(
...
# To use with python3 LightningDataModule.py &> training.log
# def main():
...
#         "train_batch_size": 64,  # batch_size = training batch size
...
#         "fp16": False,
#         "embed_batch_size": 3500,  # embedding batch size
...
#         "drop_entities": False,
#         "drop_set": False,
#         "farthest_neighbor": False,
#         "rand_gold_arbo": False,
#         "bert_model": "dmis-lab/biobert-base-cased-v1.2",
#         "out_dim": 1,
#         "pull_from_layer": 1,  # 11 for base and 23 for large
#         "add_linear": False,
#         "dataset": dataset,
#         "model": model,
...
#     MyModel = LitArboel(params=params_test)
...
#     aim_logger = AimLogger(experiment="ncbi_disease_test1")
...
#     trainer.fit(MyModel, datamodule=data_module)
